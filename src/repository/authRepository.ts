...
export const checkEmail = async (email: string): Promise<String | undefined> => {
  const [existingEmail] = await pool.query<Account[]>(`SELECT email FROM account WHERE email = ?`, [
    email,
  ]);
...
export const checkUsername = async (username: string): Promise<String | undefined> => {
...
export const createAccount = async (account: AccountPayLoad): Promise<Boolean> => {
  const [result] = await pool.query<ResultSetHeader>(
    `INSERT INTO account (username, password, email, role, create_by) VALUES (?, ?, ?, ?, ?)`,
    [account.username, account.hashedPassword, account.email, account.role, account.createBy],
  );
